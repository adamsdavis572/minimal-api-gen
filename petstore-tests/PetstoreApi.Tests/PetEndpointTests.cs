...
public class PetEndpointTests : IClassFixture<CustomWebApplicationFactory>
{
    ...
    public async Task GetPet_WithExistingId_ReturnsPet()
    {
        ...
        var addedPet = await addResponse.Content.ReadFromJsonAsync<Pet>();
    ...
    public async Task UpdatePet_WithValidData_Returns200OK()
    {
        ...
        // Update the pet
        addedPet!.Name = "Charlie Updated";
        addedPet.Status = Pet.StatusEnum.SoldEnum;

        // Act
        var response = await _client.PutAsJsonAsync("/v2/pet", addedPet);
